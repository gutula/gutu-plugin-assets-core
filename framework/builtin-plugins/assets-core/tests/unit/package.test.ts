...
import { businessFlowDefinitions, createAssetRecord, requestAssetCapitalization, issueAssetTransfer, placeRecordOnHold, releaseRecordHold, amendRecord, reverseRecord } from "../../src/flows/catalog";
...
describe("plugin manifest", () => {
  it("keeps a stable package id and business contract surface", () => {
    expect(manifest.id).toBe("assets-core");
    expect(manifest.kind).toBe("plugin");
    expect(manifest.publicCommands).toContain("assets.register.create");
    expect(manifest.publicEvents).toContain("assets.register-created.v1");
  });
});
...
describe("domain catalog", () => {
  it("keeps ERPNext parity references and operational surfaces visible", () => {
    ...
    expect(typeof createAssetRecord).toBe("function");
    expect(typeof requestAssetCapitalization).toBe("function");
    expect(typeof issueAssetTransfer).toBe("function");
