...
describe("assets-core lifecycle integration", () => {
  const previousStateDir = process.env.GUTU_STATE_DIR;
  let stateDir = "";

  beforeEach(() => {
    stateDir = mkdtempSync(join(tmpdir(), "assets-core-"));
  ...
  it("creates, advances, and reconciles a governed business record", async () => {
    const created = await createPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "assets-core:demo",
      title: "Assets Core Demo",
      ...
      correlationId: "assets-core:corr",
      processId: "asset-lifecycle:demo"
    });
    ...
    const reconciled = await reconcilePrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "assets-core:demo",
      exceptionId: "assets-core:exception",
    ...
    const amended = await amendPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "assets-core:demo",
      amendedRecordId: "assets-core:demo:amended",
      expectedRevisionNo: 5,
      title: "Assets Core Demo Amendment",
      reasonCode: "commercial-correction"
    });
    expect(amended.amendedRecordId).toBe("assets-core:demo:amended");

    const reversed = await reversePrimaryRecord({
      tenantId: "tenant_demo",
      actorId: "actor_admin",
      recordId: "assets-core:demo:amended",
      reversalRecordId: "assets-core:demo:amended:reversal",
      expectedRevisionNo: 1,
      reasonCode: "cancel-amendment"
    });
    expect(reversed.reversalRecordId).toBe("assets-core:demo:amended:reversal");
