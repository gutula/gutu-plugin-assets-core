export type AssetsCoreSqlOptions = {
  schemaName?: string;
  dropSchema?: boolean;
};

export function buildAssetsCoreMigrationSql(options: AssetsCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "assets_core", "schemaName");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getAssetsCoreLookupIndexName()} ON ${schemaName}.primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getAssetsCoreStatusIndexName()} ON ${schemaName}.exception_records (tenant_id, status, severity);`
  ];
}

export function buildAssetsCoreRollbackSql(options: AssetsCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "assets_core", "schemaName");
  const dropSchema = options.dropSchema ?? schemaName !== "assets_core";
  return [
...
export function getAssetsCoreLookupIndexName(): string {
  return "assets_core_primary_lookup_idx";
}

export function getAssetsCoreStatusIndexName(): string {
  return "assets_core_exception_status_idx";
}
