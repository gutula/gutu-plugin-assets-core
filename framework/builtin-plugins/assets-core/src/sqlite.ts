export type AssetsCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildAssetsCoreSqliteMigrationSql(options: AssetsCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "assets_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getAssetsCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getAssetsCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildAssetsCoreSqliteRollbackSql(options: AssetsCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "assets_core_");
  return [
...
export function getAssetsCoreSqliteLookupIndexName(tablePrefix = "assets_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getAssetsCoreSqliteStatusIndexName(tablePrefix = "assets_core_"): string {
